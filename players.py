...
class RandomPlayer(Player):
    ...
    def play(self, game_state):
        ...
        legal_cards = self.get_legal_actions(current_trick)
...
class BaselinePlayer(Player):
    def __init__(self, name="Baseline", cocky=36):
        super().__init__(name=name)
        self.cocky = cocky
    ...
    def suit_potential_trump_score(self, suit):
        return sum((self.card_potential_trump_score(card) if card.suit == suit else card.score) for card in self.hand)
    ...
    def get_strongest_suit(self):
        ...
        candidate_suits = [i for i, score in enumerate(potential_trump_scores) if score == max_score]
        ...
        suit_card_counts = {suit: sum(1 for card in self.hand if card.suit == suit) for suit in candidate_suits}
        max_cards = max(suit_card_counts.values())

        # Find all suits with the maximum number of cards (in case there's still a tie)
        candidate_suits_with_max_cards = [suit for suit, count in suit_card_counts.items() if count == max_cards]
    ...
    def choose_best_card(self, passed_tricks, current_trick):
        legal_cards = self.get_legal_actions(current_trick)
        remaining_cards = [card for card in Deck() if card not in sum(passed_tricks, [])]
        legal_masters = [card for card in legal_cards if self.is_master(remaining_cards, card)]
